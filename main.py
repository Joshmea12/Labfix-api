...
async def search_equipment(body: SearchRequest, user=Depends(get_current_user)):
    try:
        prompt = f"{SYSTEM_PROMPT}\n\nA lab technician is asking: {body.query}\n\nProvide helpful guidance if related to lab equipment."
...
async def copilot_chat(body: ChatRequest, user=Depends(get_current_user)):
    try:
        context_info = ""
        if body.context:
            current_tab = body.context.get("currentTab", "")
            if current_tab:
                context_info = f"\nThe technician is currently on the {current_tab} tab."

        prompt = f"{SYSTEM_PROMPT}{context_info}\n\nA lab technician asks: {body.message}\n\nProvide clear, accurate technical guidance."
        response = gemini_model.generate_content(prompt)
        return {
            "success": True,
            "response": response.text or "Could not generate a response. Please try again."
        }
    except Exception as e:
        logger.error(f"Copilot error for user {user.id}: {str(e)}")
